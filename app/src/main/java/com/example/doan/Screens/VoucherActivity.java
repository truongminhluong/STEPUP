...
public class VoucherActivity extends AppCompatActivity {
    ...
    private void setupToolbar() {
        androidx.appcompat.widget.Toolbar toolbar = findViewById(R.id.voucherToolbar);
        setSupportActionBar(toolbar);
        if (getSupportActionBar() != null) {
            getSupportActionBar().setDisplayHomeAsUpEnabled(true);
            getSupportActionBar().setDisplayShowHomeEnabled(true);
            getSupportActionBar().setTitle("Chọn Voucher");
        }
    ...
    private void setupVoucher() {
        ...
        btnDongY.setOnClickListener(v -> {
            ...
            if (selectedVoucher != null) {
                ...
                intent.putExtra("voucher_type", selectedVoucher.getType());
                intent.putExtra("voucher_value", selectedVoucher.getValue());
                intent.putExtra("voucher_minimum", selectedVoucher.getMinimumSpend());
                intent.putExtra("voucher_code", selectedVoucher.getTitle()); // tên voucher
    ...
    private void searchVoucherByCode(String code) {
        db.collection("vouchers").whereEqualTo("code", code).whereEqualTo("isActive", true).get().addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
            @Override
            public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                ...
                if (!queryDocumentSnapshots.isEmpty()) {
                    for (QueryDocumentSnapshot doc : queryDocumentSnapshots) {
                        ...
                        String expiry = new SimpleDateFormat("dd.MM.yyyy", Locale.getDefault()).format(expiryTimestamp.toDate());
    ...
    private void loadVouchersFromFirestore() {
        db.collection("vouchers").whereEqualTo("isActive", true).get().addOnSuccessListener(queryDocumentSnapshots -> {
            voucherList.clear();
            for (QueryDocumentSnapshot doc : queryDocumentSnapshots) {
                String title = doc.getString("code");
                String type = doc.getString("type");
